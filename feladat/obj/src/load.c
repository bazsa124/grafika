...
int load_model(Model* model, const char* filename)
{
    ...
    printf("[INFO] Counting the elements ...\n");
    count_elements(model, obj_file);
    printf("       vertices: %d, uvs: %d, normals: %d, triangles: %d\n",
        model->n_vertices, model->n_texture_vertices, model->n_normals, model->n_triangles);
    printf("[INFO] Allocate memory for model ...\n");
    allocate_model(model);
    printf("       done\n");
    printf("[INFO] Read model data ...\n");
    fseek(obj_file, 0, SEEK_SET);
    success = read_elements(model, obj_file);
    if (success == FALSE) {
        printf("\n[ERROR] Unable to read the model data!\n");
        return FALSE;
    }
    printf("       done\n");
    return TRUE;
